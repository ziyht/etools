 ...
 *
 */
eval evec_at   (evec v, uint idx);      // Retruns the val of specific element, returns EVAL_ZORE if not exist, see evec_val()
eval evec_first(evec v);                // Retruns the val of first    element, returns EVAL_ZORE if not exist
eval evec_last (evec v);                // Retruns the val of last     element, returns EVAL_ZORE if not exist

eval evec_val (evec v, uint idx);       // Retruns the ptr of specific element, returns 0 if not exist, see evec_at()
