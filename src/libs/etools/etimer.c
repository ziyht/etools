...
static void* __inner_loop(void* arg)
{
    ...
    while(!loop->quit)
    {
        ...
        do{
            ...
            echan_timeRecvSig(loop->sigs, 1, loop->wait_ms);
#endif
...
cstr   etimer_nowstr(cstr buf, int len)
{
...
cstr   etimer_secstr(cstr buf, int len, time_t sec)
{
    struct tm time;

    localtime_r(&sec, &time);

    snprintf(buf, len, "%4d%02d%02d%02d%02d%02d", time.tm_year + 1900, time.tm_mon + 1, time.tm_mday, time.tm_hour, time.tm_min, time.tm_sec);
...
cstr   etimer_strfstr(cstr dest, int dlen, constr dfmt, constr from, constr ffmt)
{
    ...
    return strptime(from, ffmt, &_tm) ? strftime(dest, dlen, dfmt, &_tm) ? dest
                                                                         : 0
                                      : 0;
}

cstr   etimer_strfsec(cstr dest, int dlen, constr dfmt, i64   stamp)
{
    struct tm _tm;

    return localtime_r(&stamp, &_tm) ? strftime(dest, dlen, dfmt, &_tm) ? dest
                                                                     : 0
                                  : 0;
}

i64    etimer_strpsec(constr from, constr ffmt)
{
    struct tm _tm = {0};

    return strptime(from, ffmt, &_tm) ? mktime(&_tm)
                                      : -1;
}
...
i64    etimer_strpms (constr from, constr ffmt)
{
    ...
    return strptime(from, ffmt, &_tm) ? mktime(&_tm) * 1000
                                      : -1;
}

static inline __attribute__((always_inline)) cstr __elapsestrftm(cstr buf, int len, constr fmt, struct tm* time)
{
    ...
    return strftime(buf, len, fmt, time) ? buf : 0;
}

static inline __attribute__((always_inline)) i64 __elapsesecffmt(constr from, constr ffmt)
{
...
cstr   etimer_elapsefstr(cstr dest, int dlen, constr dfmt, constr from, constr ffmt)
{
   ...
   return etimer_elapsefsec(dest, dlen, dfmt, elapse_sec);
}
...
{
    sockaddr_in addr; int sockfd; struct timeval wait, end; fd_set inset; _ntp_packet_t req_pack, res_pack; int diff_sec, diff_usec;//, delay_sec, delay_usec;
    esync out = {0,0,0,0};
    ...
    // -- recieve response
    if(select(sockfd + 1, &inset, NULL, NULL, &wait) < 0)
    {
        goto ret;
    }
    else
