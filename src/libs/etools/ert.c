...
static ert _df_tp;
static int _df_thread_num = MAX_THREAD_NUM;
...
ert  ert_new(int max_thread_num)
{
    ...
    thread_init(tp->quit_th, _etp_quit_cb, tp);
    if(!tp->quit_th)
        goto err_ret;

    tp->thrds_cap = max_thread_num == 0 ? MAX_THREAD_NUM : max_thread_num;
...
void ert_release(ert tp, int opt)
{
    ...
    tp->quit_join_ths = opt & ERT_WAITING_TASKS;
...
static void _etp_quit_cb(void* _tp)
{
    ert tp; ejson itr; TH th; TASK t; thread_t tp_quit_th;
...
{
    TH th; ert tp; ejson te; TASK t; char tag[16];
