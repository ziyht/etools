...
#undef ETHREAD_USING_PTHREAD_MINGEWIN  // setting for ethread on win32
...
static int __elog_rtt_check(_elogh h, i64 nows)
{
    ...
    {
        struct tm time; time_t sec; int basic_len;
...
static int __elog_fflush_to_file(_elogh h)
{
    ...
    if(h->fd >= 0)
    {
        ret = write(h->fd, h->msgs, estr_len(h->msgs));
...
{
    _elogh h; _elog _e; time_t s; i64 ns; char _us[10]; struct tm lt; int fmtlen, timelen, uslen, i = 0; char timestr[ELOG_TIMESTR_MAXLEN]; va_list ap;
...

    {
        ...
        timelen = strftime(timestr, ELOG_TIMESTR_MAXLEN, h->time_fmt, &lt);

        if(h->rtt.enable) __elog_rtt_check(h, s);
    }

    h->last_msg_pos = estr_len(h->msgs);
    h->tag_poss     = 0;
    //memset(h->tag_pos, 0, h->tag_poss * sizeof(int));

    op     = NORMAL;
    uslen  = 0;
    fmtlen = sstr_len(h->elog_fmt);
    for (; i < fmtlen; ++i) {
        if (op == NORMAL) {
            if (h->elog_fmt[i] == '%')
                op = SUBST;
            ...
            switch (h->elog_fmt[i]) {
                ...
                case 'l':
                           ...
                           if(h->tag_poss < ELOG_MAX_TAGS) h->tag_pos[h->tag_poss++] = estr_len(h->msgs);
#endif
