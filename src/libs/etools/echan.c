...
static inline _queue _queue_new(size_t capacity)
{
...
typedef int  (*__echan_sendBin_cb)(echan chan, conptr in, size_t inlen);
...
static int __chan_init_buffered(echan chan, int opts, size_t capacity)
{
    ell  l = 0;
...
recv_again:
    ...
    while (ell_len(chan->channal.list) == 0)
    {
        if (chan->status != _OPENDED)
        {
            emutex_ulck(chan->m_mu);
...
static int __echan_sendBin_list(echan chan, conptr in , size inlen) {                                  eobj obj = ell_newO(ERAW, inlen);              is0_ret(obj, 0); memcpy(obj, in, inlen);     return chan->_oprt->send(chan, (eval_t){.p=obj});}
...
static int __echan_sendStr_list(echan chan, constr str            ) { int  len = str ? strlen(str) : 0;eobj obj = ell_newO(ESTR, len);                is0_ret(obj, 0); memcpy(obj, str, len);      return chan->_oprt->send(chan, (eval_t){.p=obj});}
...
static int __echan_sendBin_sigs(echan chan, conptr in , size inlen) { fprintf(stderr, "__echan_sendBin_sigs: wrong use of echan, you chan only using this in a LIST chan\n"); abort(); }
...
static int __echan_get_oprt(echan chan, int opts, size capacity)
{
...
echan echan_new(int opts, size capacity)
{
    echan chan;
...
int  echan_sendB(echan chan, conptr in , size inlen) { return chan ? chan->_oprt->sendBin(chan, in, inlen) : 0; }
