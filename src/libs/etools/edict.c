...
#define _c_keys(r)          _r_keys(_c_r(d))
#define _c_ht(d)            d->ht
...
static u64 _edict_fingerprint(edict d) {
    ...
    integers[0] = (long) _c_ht0(d).table;
    integers[1] = _c_ht0(d).size;
    integers[2] = _c_ht0(d).used;
    integers[3] = (long) _c_ht1(d).table;
...
static inline ulong __edict_nextPower(ulong size)
{
...
static inline int __edict_expand(edict d, ulong size)
{
    _edictht_t n;
    ulong realsize = __edict_nextPower(size);
...
{
    unsigned int h, idx, table; _edictn he;
...
eobj  edict_val (edict d, ekey key) { return   edict_find(d, key); }
i64   edict_valI(edict d, ekey key) { eobj o = edict_find(d, key); _eo_retI(o); }
f64   edict_valF(edict d, ekey key) { eobj o = edict_find(d, key); _eo_retF(o); }
cptr  edict_valR(edict d, ekey key) { eobj o = edict_find(d, key); _eo_retR(o); }
cptr  edict_valP(edict d, ekey key) { eobj o = edict_find(d, key); _eo_retP(o); }
cstr  edict_valS(edict d, ekey key) { eobj o = edict_find(d, key); _eo_retS(o); }

etypeo edict_valType  (edict d, ekey key) { eobj o = edict_find(d, key); _eo_retT(o); }
uint   edict_valLen   (edict d, ekey key) { eobj o = edict_find(d, key); _eo_retL(o); }
bool   edict_valIsTrue(edict d, ekey key) { return __eobj_isTrue(edict_find(d, key)); }

bool  edict_isEmpty(edict d) { if(d) return _c_len(d) == 0; return true; }
uint  edict_len    (edict d) { if(d) return _c_len(d)     ; return    0; }
uint  edict_size   (edict d) { if(d) return _c_len(d)     ; return    0; }
...
{
    unsigned int h, idx; int table; _edictn he, prevHe;
...
{
    unsigned int h, idx; int table; _edictn he, prevHe, n;
