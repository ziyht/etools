...
int edict_val_test()
{
...

    {
        ...
        eexpect_num(edict_len(d), 5);

        i = 1;
        o = edict_val(d, ekey_i(i));
        ...
        i = 1;
        eexpect_num(eobj_typeo(edict_val(d, ekey_i(i))), ENUM);
        eexpect_num(edict_valI(d, ekey_i(i)), 1);
        eexpect_num(edict_valF(d, ekey_i(i)), 1.0);

        i = 2;
        eexpect_num(eobj_typeo(edict_val(d, ekey_i(i))), ENUM);
        eexpect_num(edict_valI(d, ekey_i(i)), 2);
        eexpect_num(edict_valF(d, ekey_i(i)), 2.0);

        i = 3;
        eexpect_num(eobj_typeo(edict_val(d, ekey_i(i))), EPTR);
        eexpect_ptr(edict_valP(d, ekey_i(i)), (cptr)3);

        i = 4;
        eexpect_num(eobj_typeo(edict_val(d, ekey_i(i))), ESTR);
        eexpect_str(edict_valS(d, ekey_i(i)), "4");

        i = 5;
        eexpect_num(eobj_typeo(edict_val(d, ekey_i(i))), ERAW);
        eexpect_raw(edict_valR(d, ekey_i(i)), "\0\0\0\0\0", 5);
        ...
        for(i = 0; i < 100; i++)
        {
            eexpect_num(edict_valI(d, ekey_i(i)), i);
        }
    }

    {
        ...
        /// -- test 2
        i = 1;
        eexpect_num(eobj_typeo(edict_val(d, ekey_s(&key[i]))), ENUM);
        eexpect_num(edict_valI(d, ekey_s(&key[i])), 1);
        eexpect_num(edict_valF(d, ekey_s(&key[i])), 1.0);

        i = 2;

        eexpect_num(eobj_typeo(edict_val(d, ekey_s(&key[i]))), ENUM);
        eexpect_num(edict_valI(d, ekey_s(&key[i])), 2);
        eexpect_num(edict_valF(d, ekey_s(&key[i])), 2.0);

        i = 3;
        eexpect_num(eobj_typeo(edict_val(d, ekey_s(&key[i]))), EPTR);
        eexpect_ptr(edict_valP(d, ekey_s(&key[i])), (cptr)3);

        i = 4;
        eexpect_num(eobj_typeo(edict_val(d, ekey_s(&key[i]))), ESTR);
        eexpect_str(edict_valS(d, ekey_s(&key[i])), "4");

        i = 5;
        eexpect_num(eobj_typeo(edict_val(d, ekey_s(&key[i]))), ERAW);
        eexpect_raw(edict_valR(d, ekey_s(&key[i])), "\0\0\0\0\0", 5);
        ...
        {
            sprintf(key, "%d", i);
            eexpect_num(edict_valI(d, ekey_s(key)), i);
        }
    }
...
int edict_itr_test()
{
...

    {
        ...
        {
            sprintf(buf, "%"PRId64"", i+100000000000);
            eexpect_str(edict_valS(h, ekey_i(i)), buf);
            i++;
        }
        edict_freeItr(ditr);
    }

    {
        ...
        edict_foreach(ditr, obj)
        {
            sprintf(key, "%d", i);
            sprintf(buf, "%"PRId64"", i+100000000000);
            eexpect_str(edict_valS(h, ekey_s(key)), buf);
            i++;
        }
        edict_freeItr(ditr);
    }
