...
{
    erb h = erb_new(EKEY_I);  u64 t, tn; uint j, len; i64 i;
    ...
    t = eutils_nowms();
    len = erb_len(h); j = 0; scale ++;

    tn = eutils_nowns(); i = 0;
    if(erb_freeOne(h, (ekey){i})) j++;
...
{
    erb h = erb_new(EKEY_S);  char keyS[16]; u64 t; uint j, len;

    t = eutils_nowms();
    for(i64 i = 0; i < scale; i++)
    {
        sprintf(keyS, "%"PRIi64"", i);
        erb_addR(h, ekey_s(keyS), 0);
    }
    ...
    {
        sprintf(keyS, "%"PRIi64"", i);
        if(erb_freeOne(h, ekey_s(keyS))) j++;
    }
...
{
    erb h = erb_new(EKEY_I);  int j; i64 i;
