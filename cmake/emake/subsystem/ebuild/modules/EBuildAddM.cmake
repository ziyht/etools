...
#
#
#   1. 设置 option ${category}_${name} 为 ${default}
#   2. 添加 ${name} 到 _G_${category}S_
#   3. 检查 ARGN, 若逻辑表达式为 true, 将强制设置 ${category}_${name} 为 ON
#
macro(_mAddBuild category name doc default)

    cmake_parse_arguments(M "CREATE;DELETE" "INDEX;PROD;" "" ${ARGN})
    ...
    if(M_DELETE)
        ...
        option(${category}_${_name} "${doc}" ${default})
        set(${category}_${_name} ${default})
...
#
# \! 添加 外部库
#   1. 设置 option EXT_${name} 为 ${default}
#   2. 添加 ${name} 到 EXTS 中
#
macro(EBuildAddExtM name doc default)

    _mAddBuild(EXT ${name} "${doc}" ${default} ${ARGN})

endmacro()

#
# \! 添加 lib
#   1. 设置 option LIB_${name} 为 ${default}
#   2. 添加 ${name} 到 LIBS 中
#
macro(EBuildAddLibM name doc default)

    _mAddBuild(LIB ${name} "${doc}" ${default} ${ARGN})

endmacro()

#
# \! 添加 plugin
#   1. 设置 option PLG_${name} 为 ${default}
#   2. 添加 ${name} 到 PLGS 中
#
macro(EBuildAddPlgM name doc default)
    _mAddBuild(PLG ${name} "${doc}" ${default} ${ARGN})
endmacro()

#
# \! 添加 app
#   1. 设置 option APP_${name} 为 ${default}
#   2. 添加 ${name} 到 APPS 中
#
macro(EBuildAddAppM name doc default)
    _mAddBuild(APP ${name} "${doc}" ${default} ${ARGN})
endmacro()

#
# \! 添加产品和版本
#   1. 根据 name 解析 prod 和 type，如：product1：type1 解析 product为 product1，type 为 type1
#   2. 设置 option PROD_${prod} 和 TYPE_${type} 为 ${default}
...
#
macro(EBuildAddProdM name doc default)
    ...
    if(_type)
        ...
        if(${_o_index} EQUAL -1)
            _mAddBuild(PROD ${_prod} "${doc}" ${default} ${ARGN})
        endif()

        _mAddBuild(TYPE ${_type} "${doc}" ${default} PROD ${_prod} ${ARGN})
